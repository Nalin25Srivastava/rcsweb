...
const Signup = () => {
    ...
    const loginWithGoogle = useGoogleLogin({
        onSuccess: (tokenResponse) => {
            console.log('Google Signup Success (Access Token):', tokenResponse);
        ...
        onError: (error) => {
            ...
            if (error.error === 'idpiframe_initialization_failed') {
                console.error('Google Auth initialization failed. Check GCP origin whitelist.');
            }
        }
    });
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="flex flex-col items-center gap-4 mt-6 w-full">
                            <button 
                                ...
                                className="w-full max-w-[350px] flex items-center justify-center gap-3 bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 py-3.5 px-6 rounded-full shadow-sm hover:shadow-md hover:border-blue-500/30 transition-all active:scale-95 group cursor-pointer"
                            >
